...
> 所有需求交付的强制基线。详见 ADR-0020 与 `CLAUDE.md`。顺序不可打乱。

```
① 软件工程师从 origin/master 切特性分支 → 需求规格 + 改代码 + 同 PR 写测试
② 全部测试通过：单测(Vitest/pytest) + 集成 + E2E(Playwright，生产配置)
③ CI 确定性门禁全绿 → AI 代码审查助手预审（advisory，只留评论不合并）
④ 🧍人类审核者 Approve → CI 自动 squash 合并 master（AI 不自行合并）
⑤ 🧍等待人工(QA/陈豪)回复"确认上线"
⑥ 基于 master 自动构建 → 部署 → 冒烟测试 → 失败自动回滚
```

**五条硬规则：**

1. **master 起步**：修改从 `origin/master` 切分支，PR 目标恒为 `master`，不在特性分支上叠特性。
2. **测试全过含 E2E**：面向用户的功能必须同时有代码级单测**和**浏览器级 E2E（Playwright，用生产配置）。E2E **不可跳过，不可用接口测试或组件单测替代**。无 E2E 覆盖的需求不得升到「✅ 已验收」，不得进入部署。
3. **测试与文档跟随工程**：测试用例、需求规格锚点、ADR（`docs/decisions/`）随代码同 PR 提交、同 PR 合并 master；不单次使用、不留特性分支。回归直接拉 master 复用全部历史用例。
4. **合并前置部署**：部署必须在 PR 合并 master **之后**、基于 master 构建；**特性分支不直接上线**。
5. **禁止 AI 审 AI 合并**：验证靠确定性 CI；AI 代码审查助手只做 advisory 预审，**合并批准权在人类审核者**，上线确认权在用户。

> Bug 验证路由（AI-Native）：软件工程师先写复现测试（先红后绿）再修复，同 PR 提交；由确定性 CI（含 E2E）验证，不再交叉指派给独立测试智能体。

---

## 自动化流程

### 质量门禁（PR 阶段，ai-native-pipeline.yml）
...
1. ✅ 前端类型检查 + Lint + 单元测试
2. ✅ 后端单元 + 集成测试
3. ✅ E2E（Playwright，生产配置）
4. ✅ 覆盖率 ≥ 70%
5. 全绿 → AI advisory 预审 → 请求人类审核 Approve

### CI（master 合并后兜底，ci.yml）

**触发条件**：push 到 master

**自动执行**：类型检查 + Lint + 单元/集成测试（PR 已跑过，此为 master 直推兜底）

### CD（持续部署，ai-native-pipeline.yml）

**触发条件**：部署确认 Issue 中用户回复"确认上线"

**自动执行**：
1. ✅ 基于 master 构建 → SSH 部署到生产服务器
2. ✅ 冒烟测试
3. 🚨 失败自动回滚到上一版本
