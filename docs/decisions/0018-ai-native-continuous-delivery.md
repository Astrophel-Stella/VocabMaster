...
## 状态
已被取代 (Superseded by ADR-0020, 2026-07-31)

> ⚠️ 本 ADR 中"3 个角色（产品工程师/质量工程师/架构师）"及"架构师自动合并"的设计**已被 ADR-0020 取代**。
> 取代原因：仍受多智能体流水线思路影响，两处不符合国际最佳实践——
> (1) 把测试拆成独立"质量工程师"Agent（应由软件工程师同 PR 写测试，CI 验证）；
> (2) 让"架构师 Agent 自动合并"（禁止 AI 审 AI 合并到生产，合并闸口必须是人类）。
> 现行权威设计见 [ADR-0020](0020-single-swe-agent-human-gated-merge.md)。以下内容仅作历史保留。
