...
Tauri 可打包为 MSI(WiX)或 NSIS(.exe 安装器)。本应用 `productName` 为「VocabMaster」。
...
- **WiX 的 `light.exe` 在生成 en-US 的 MSI 时,遇到非 ASCII 应用名可能失败**(编码兼容性问题)。
- NSIS 对国际化支持良好,且更轻、不依赖 WiX 工具链。
